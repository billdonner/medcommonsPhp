...
/// need to return guids
	...
	function xmlbody(){
		...
		$query = "SELECT rights.user_medcommons_user_id,rights.document_ID,rights.rights_id,rights.accepted_status,document.guid  from rights INNER JOIN document
							ON (document.id = rights.document_ID)  WHERE (user_medcommons_user_id='$mcid')";
		...
		while ($l = mysql_fetch_row($result)) {
			...
			$this->xm("<entry >");
			$this->xm("<mcid>".$l[0/*'user_medcommons_user_id'*/]."</mcid>"); // preserve compatibility

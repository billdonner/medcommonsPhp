...
def get_object_access_control_policy(bucket, object, key_id=None, secret=None):
...
def put_data(bucket, name, data, content_type='application/octet-stream',
    ...
    data_to_url('PUT', '/%s/%s' % (bucket, name), data, content_type, headers)

def devpay_headers(user_token, product_token):
    if user_token and product_token:
        if not user_token.startswith('{UserToken}'):
            user_token = '{UserToken}' + user_token
        if not product_token.startswith('{ProductToken}'):
            product_token = '{ProductToken}' + product_token
        return {'x-amz-security-token': '%s,%s' % (user_token, product_token)}
...
def data_to_url(method, url, data, content_type=None, headers=None):
    if content_type:
	hash = md5.new(data)
    else:
	hash = None
    ...
    if r.status / 100 != 2:
	raise "%s %s %s" % (r.status, r.reason, r.read())

    return r

def rest_headers(method, url, hash, content_type, headers=None,
                 key_id=None, secret=None):
    headers = headers or {}
    x_amz_headers = ['%s:%s' % (k.lower(), v) \
    ...
    h = hmac.new(secret or SECRET, digestmod=sha)
    to_sign = [method, md5_hash, content_type, date] + x_amz_headers

...
function userinfo($accid,$glevel)
{
  ...
  // ssadedin: gobs of errors caused by misc nullable user and account values 
  // all praise to e_strict
  if(!isset($u->email)) $u->email = "";
  if(!isset($u->sha1)) $u->sha1 = "";
  if(!isset($u->server_id)) $u->server_id = "";
  if(!isset($u->since)) $u->since = "";
  if(!isset($u->first_name)) $u->first_name = "";
  if(!isset($u->middle_name)) $u->middle_name = "";
  if(!isset($u->last_name)) $u->last_name = "";
  if(!isset($u->mobile)) $u->mobile = "";
  if(!isset($u->smslogin)) $u->smslogin = "";
  if(!isset($u->updatetime)) $u->updatetime = "";
  if(!isset($u->ccrlogupdatetime)) $u->ccrlogupdatetime = "";
  if(!isset($u->photoUrl)) $u->photoUrl = "";
  if(!isset($u->chargeclass)) $u->chargeclass = "";
  if(!isset($u->rolehack)) $u->rolehack = "";
  if(!isset($u->affiliationgroupid)) $u->affiliationgroupid = "";
  if(!isset($u->startparams)) $u->startparams = "";
  if(!isset($u->stylesheetUrl)) $u->stylesheetUrl = "";
  if(!isset($u->picslayout)) $u->picslayout = "";
  if(!isset($a->comment)) $a->comment = "";
  if(!isset($a->address1)) $a->address1 = "";
  if(!isset($a->address2)) $a->address2 = "";
  if(!isset($a->city)) $a->city = "";
  if(!isset($a->state)) $a->state = "";
  if(!isset($a->postcode)) $a->postcode = "";
  if(!isset($a->country)) $a->country = "";
  if(!isset($a->telephone)) $a->telephone = "";
  if(!isset($a->DOB)) $a->DOB = "";
  if(!isset($a->Age)) $a->Age = "";
  if(!isset($a->Sex)) $a->Sex = "";

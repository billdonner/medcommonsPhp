 ...
 *
 *   1. (request_token,url) = authorize( key, secret, healthurl, callback ) will redirect the browser to an authorization page
 *
 *   2. ...(redirect to provided url for authorization page)...
 *
 *   3. // On callback page
 *      api = confirm_authorization(key, secret, request_token->key, request_token->secret, healthurl)
 *
...
class ApplianceApi {
  ...
  public function __construct($consumer_token, $consumer_secret, $appliance_url, $access_token=null, $access_secret=null) {
    ...
    $this->appliance_url = $appliance_url;
  }
  ...
  public function get_request_token($accid) {
    ...
    $result =  $this->call($this->appliance_url . "api/request_token.php", array());
    dbg("Received request token $result");
    $req_token = array();
    parse_str($result,$req_token);

    if(!isset($req_token['oauth_token'])) {
      error_log("Failed to retrieve request token from ".$req->to_url()." result = ".$result);
  ...
  public function call($url, $params) {
    ...
    dbg("executing OAuth call: ".$req->to_url());

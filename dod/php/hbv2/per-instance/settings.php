...
function insertUser($fbuid,$firstName,$lastName, $sex,$city,$state,$country,$photoUrl,$key,$product) {
	...
	$appcallbackurl = $GLOBALS['base_url'];
	$appname = $GLOBALS['healthbook_application_name'];
	dbg("Received auth $auth and secret $secret for new patient $mcid");
	logHBEvent($fbuid ,'added',"Added  fbid $fbuid mcid $mcid  $appname");
	mysql_query("REPLACE INTO fbtab (fbid,mcid,applianceurl,targetfbid,targetmcid,groupid,oauth_token,oauth_secret,appcallbackurl)
	VALUES ('$fbuid','$mcid','$applianceUrl','$fbuid','$mcid',NULL,'$auth','$secret','$appcallbackurl')") or die("error inserting into fbtab: ".mysql_error());
	return $mcid;
}
...
function dissociateUser($user)
{
	...
	if ($r) {
		...
		$q ="delete from careteams  where fbid='$user' "; // or giverfbid='$user' ";
		$result = mysql_query($q) or die("cant   $q ".mysql_error());
		...
		$q = "update fbtab set mcid='0',targetfbid='0',targetmcid='0', oauth_token = NULL, oauth_secret = NULL, storage_account_claimed = 0 where fbid='$user'";// and mcid = '$mcid'";
		$result = mysql_query($q) or die("cant   $q ".mysql_error());
...
{
	$appcallbackurl = $GLOBALS['baseurl'];
	$q = "replace into  fbtab set mcid='0', targetfbid='0', targetmcid='0', appcallbackurl='$appcallbackurl', fbid='$user'";// and mcid = '$mcid'";
	$result = mysql_query($q) or die("cant   $q ".mysql_error());
...
if (isset($_REQUEST['newacct']))
{
	...
	logHBEvent($user,'amz',"Off to amazon $returUrl");

...
<fb:tabs>
      ...
      <fb:tab_item href='documents.php?o=t' title='add document' />
...
XXX;
	...
	$appname = $GLOBALS['healthbook_application_name'];
	$dash = content_dashboard($me,'documents');
	ob_start();
?>
<fb:fbml version='1.1'>
  <?=$dash?>
  ...
  <fb:explanation>
    <fb:message>Documents in  <fb:name uid='<?=$user?>' possessive=true  linked=false useyou=true /> <?=$appname?> Account</fb:message>
    Current documents are listed below:
    <fb:iframe src='<?=$u->appliance?>/acct/accountDocuments.php?t=widget&accid=<?=$u->targetmcid?>' smartsize='true' frameborder='0'/>
  </fb:fbml>
<?
...
//	if ($mcid===false) die ("Internal error, fb user $user has no mcid");
	switch ($op)
	{
		...
		default : {  $markup =acoverview($user,$t->fbid,$t) ;break;}

...
class HealthBookUser {
  ...
  public function authorize($url) {
    ...
    // TODO: this should really be signing the urls properly using OAuth.
    // For now we cheat and just pass the auth parameter which works.
    if(strpos("$url","?")===FALSE)
      $url .= "?";
    else
      $url .= "&";
    ...
    $result.= "&identity_type=FaceBook".
      "&identity=".$this->fbid.
      "&identity_name=".urlencode($this->getFirstName()." ".$this->getLastName());
  ...
  private function loadProfileInfo() {
    ...
    if($this->profileInfo == null) {
      $infos = ($facebook->api_client->users_getInfo($this->fbid,array('first_name','last_name','pic_small','sex','current_location')));
  ...
  public static function load($fbid) {
   ...
   //echo "in load fbid is $fbid mcid is $u->mcid";
    ...
    $hbc->appliance = $u->applianceurl ? $u->applianceurl : false;

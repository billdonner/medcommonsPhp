...
$GLOBALS['Payments_Url'] = "https://ondemand.medcommons.net/mod/";
...
function bval ($bkt)
{
	return  mysql_fetch_object(dosql(" SELECT * FROM `payment_stats` WHERE `key` = '$bkt'  "))->accumulator;
}
function bump($bkt,$quantity)
{
	$newamount = bval($bkt)+$quantity;
	dosql("Update `payment_stats` set accumulator = '$newamount' WHERE `key` = '$bkt' ");
}
...
if (isset($_GET['info']))
{
	$totalamount = doprettyprice(bval('total_amount'));
	$totaltrans = bval('total_payments');
	
	if (isset($_GET['html']))
	{
		$sleepsecs = $_GET['html'];
		echo "
		<html><heald><title>MedCommons Payment Services Operations Status</title>
		<meta http-equiv='Refresh' content='$sleepsecs' /></head><body>
		";
	echo  ("transactions: $totaltrans  amount: $totalamount </body></html>");
		exit;		
	}
	...
	<transactions>$totaltrans</transactions>
...
<p>Please wait while we contact our card processor...</p>

...
function dwpage($leagueind,$my_role,$body)
{
	...
	$title = "Query Data Warehouse for League - $rr->name";
...
&nbsp;|&nbsp;<a href='signon.php?logout=dw.php' title='logout from informed sports'>logout</a>
</div>
$userpagefooter
XXX;
	$outbuf = <<<XXX
	...
	<div class=teampanel><div class=teaminfo>Refine search parameters - all checked parameters must match</div></div>
	<div id='is_body'>
	<div id='is_c_section'>
	$body
	</div>
...
function qbeform($plugid,$leaguename,$r,$x)
{
	...
	This query finds all  players in $league->name  whose corresponding fields in $rr->label are checked and match any non-blank values. 
	</p>
	...
	$qbeform .= "</table><input type=submit name=submit value = 'Find' />
		</form>
		</div>
	";			
	return $qbeform;

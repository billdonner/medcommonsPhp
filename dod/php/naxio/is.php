...
<div  class="clearfix">
		<h1 class="logo-text">
			<a href="http://axiosports.com/index.php" target=showframe title="AxioSports"><span>AxioSports Navigator 0.2</span></a>	
		</h1>
...
if (!isset ($_COOKIE['u']))
{
	...
	if (isset($GLOBALS['openid_hack'])){
		...
		if (isset($_GET['openid_ns']) || isset($_GET['openid_mode'])) {
			...
			*/
			...
			if ($response->status != Auth_OpenID_SUCCESS) {
				...
				$url = 'signon.php?err=Verification+Cancelled';
				else if ($response->status == Auth_OpenID_FAILURE)
				$url = 'signon.php?err=OpenID+Authentication+Failed';
				else
				$url = 'signon.php?err=Unknown+OpenID+Error';
		...
		if ($r===false)
		{
			...
			$url ="signon.php?err=OpenID+Not+Found+on+IS+" . urlencode($openid);
		}
		else
		{
			...
			switch ($r->role)
			{
				...
				default :{$url = "signon.php?err=bad+role"; break;}
			}
		}
		if ($url!='') {
			redirect ($url);

 ...
 *
 */
public class MesaSN11182Action extends MesaBaseAction implements AtnaConstants{

	
	
	private static Logger logger = Logger.getLogger(MesaSN11182Action.class);
	 public void executeTest() throws Exception{
		
	     setTestname("SN 11182");
      ...
      for(int i = 0; i < fRhios.length; i++){
          try{
            ...
            PatientIdType idType = new PatientIdType();
          	...
          	String queryText = "SearchPatient:FamilyName=MOORE";
        	String registryID = fRhios[i].getPixConfig().getPixServerProperties().getApplication();
            logger.info("About to register event with registry id " + registryID);
          	String dataRecipientId = "dataRecipientId";
          	//response = binding.auditPatientRecordReadEvent(sessionContext, eventOutcome, patientId);
          
          	response = binding.auditActorStopEvent(sessionContext, eventOutcome);
          	String message = null;

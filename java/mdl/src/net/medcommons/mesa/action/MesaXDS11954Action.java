...
import OHFBridgeStub.FindDocumentQueryType;
 ...
 *
 */
public class MesaXDS11954Action extends MesaBaseAction {

	private static Logger logger = Logger.getLogger(MesaXDS11954Action.class);

	public void executeTest() throws Exception {

		setTestname("XDS 11954");
		...
		FindDocumentQueryType queryDocumentQueryType = new FindDocumentQueryType();
		//queryDocumentQueryType.setPatientID()
		/*
		 * FindDocumentQueryType docQuery = new FindDocumentQueryType();
        PatientIdType idType = new PatientIdType("4bb0cd5c4a7245f");
        idType.getAssigningAuthorityType().setUniversalId("1.3.6.1.4.1.21367.2005.3.7");
        idType.getAssigningAuthorityType().setUniversalIdType("ISO");  
    	docQuery.setPatientID(idType);
    	
    	docQuery.setClassCodes(new String[]{"Consult"});
    	docQuery.setHealthCareFacilityCodes(new String[]{"Outpatient"});
		 */
		...
		for (int i = 0; i < fRhios.length; i++) {
			try {
				...
				response = binding.queryDocumentsFind(sessionContext, queryDocumentQueryType, queryPreferences);
		...
		logger.info(log);
		 for(int i = 0; i < fRhios.length; i++){
	        	...
	        	for (int j=0;j<documents.length; j++){
	        		...
	        		XdsDocType aDocument = documents[j];
	        		
	        		//documents[j].getMimeType();
	        		if (uri == null){

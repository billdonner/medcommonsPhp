...
function getFavLink($user,$ord)
{
	...
	if ($b===false) return "<a class='tinylink' href='topics.php?addfav&ord=$ord' >add to your favorites</a>";
	else return "<a class='tinylink' href='topics.php?remfav&ord=$ord' >remove from your favorites</a>";
}
...
function getGroupBadge($facebook,$gid,$pic,$linked,$exclude,$reford,$subhead)
{
	...
	while ($r  = mysql_fetch_object($result))
	{
		...
		{
			$span= <<<XXX
		<span><a href='topics.php?ord=$r->nlmord' title="gid:$gid topic:$r->nlmord by:$r->creatorfbid $r->created"  >$r->nlmtopic<span class=hbcounter>($r->hurlcount)</span></a>  
...
if ($linked) $alink = "<a href=groups.php?gid=$gid&ord=$reford >$name<span class=hbcounter>($gurlcount)</span></a>";
else $alink="$name<span class=hbcounter>($gurlcount)</span>";
$badge .= <<<XXX
  <fb:explanation>
...
function getHealthBookGroups($facebook, $user,$targetfbid)
{
	...
	$markup = <<<XXX
  <fb:explanation>
    <fb:message>$my Facebook Groups with Public HealthURLs <a class=tinylink href='topics.php'>more...</a></fb:message> 
   $arGroup
  </fb:explanation>
</fb:fbml>
XXX;
...
$outstr =" <div class='mugshots'><fb:explanation>
          <fb:message>$my Facebook Groups with Public HealthURLs <a class=tinylink href='home.php?o=x'>more...</a></fb:message><table><tr>";
$counter = 0;
...
function getRecentHealthBookGroups($facebook)
{
	...
	$message = "Facebook Groups with Recent Public HealthURL Activity";
	return getHealthBookGroupsInt($facebook,$q,$message,$morelink);
}
...
XXX;
	$markup = <<<XXX
  <fb:explanation>
    <fb:message>Facebook Groups with Recent Public HealthURL Activity for $topic</fb:message> 
   $arGroup
  </fb:explanation>
</fb:fbml>
XXX;
...
posted Health URL <span><a target='_new'  href='$rr->hurl' ><img src="http://www.medcommons.net/images/icon_healthURL.gif" />$rr->hurl</a></span> to topic <a href='topics.php?ord=$rr->topic' >$rr->nlmtopic<span class=hbcounter>($rr->hurlcount)</span></a>$name 
...
{
	$message ="Recently Published Public HealthURLs By <img src='http://static.ak.facebook.com/images/icons/group.gif?48:25796' /><fb:grouplink gid=$gid />";;

...
function my_role()
{
	if (!isset($_COOKIE['u']))
	{
		redirect("signon.php?logout=checkloginnocookie");
		//echo "redirecting to signon.php?logout";
		//exit;
	}
	...
	if (!$r)
	{
		redirect("signon.php?logout=nouser");
		//echo "redirecting to signon.php?logout&err=nouser";
		//exit;
	}
...
function check_login($roles,$blurb)
{
	...
	redirect("signon.php?logout=badrole+$urole+$roles");


	//echo "redirecting to signon.php?logout&err=badrole";
	//exit;
}
...
{ // starts with tail end of the linkarea section
	...
	;
	$ret = <<<XXX
 <div id='footer'>$time - you are signed on as $myid; your role is: $my_role<br/><a href="signon.php" title="Axio Sports">Axio Sports</a>
...
&nbsp;|&nbsp;<a href='signon.php?logout=footer' title='logout from informed sports'>logout</a>
</div>
$userpagefooter
XXX;

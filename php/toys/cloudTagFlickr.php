...
Version 1.6 - Per Save the Dave's request I added a "good words only" flag.  Set them in the "onlyusethese" array, and turn on the feature using a "focuson=true" flag in the URL.
...
$flickrapikey = "flickrkey"; //key in here
...
$onlyusethese[]="berkeley";
$onlyusethese[]="friends";
$onlyusethese[]="japan";
...
$dbUser = "dbuser";//database user here
$dbPass = "dbpass";//database password here
$dbAddress = "localhost"; //location of database
$dbTable = "dbTable";//location of table, will create one this is it doesn't exist
...
if (isset($_GET['mintagcounthreshold']) and ($_GET['mintagcounthreshold'] != "")){
	$mintagcounthreshold = $_GET['mintagcounthreshold'];
	}
...
if (!empty($username)) {
   ...
   //Drill Down to Pertinent Information
	...
	if ($togo['count'] >= $mintagcounthreshold)
	{
		...
		if ($_GET['focuson']=='true')
		{
			foreach ($onlyusethese as $goodword)
			{ if (strtolower($togo['_value'])==strtolower($goodword)) $a++; }
			if ($a > 0) $goForward = true;
		}
		else
		{
